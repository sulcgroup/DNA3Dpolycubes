...
from .effect import Effect, StringConditionalEffect, DynamicEffect, EFFECT_CLASSES
from ..util import rotAroundAxis, powerset

from ..util import from_xyz, to_xyz, getSignedAngle
from ..patchy_base_particle import PatchyBaseParticleType, BasePatchType, BaseParticleSet
...
def diridx(a: np.ndarray) -> int:
    """
    :param a: a 3-length integer vector with magnitude 1
    :raise ValueError: if param a is not a 3-length integer vector with magnitude 1
    :return: the direction index in RULE_ORDER of a vector
    """
    for idx, rule in enumerate(RULE_ORDER):
        if np.array_equal(a, rule):
            return idx
    raise ValueError(f"{a} is not in RULE_ORDER")

def rdir(d: int) -> int:
    """
    :param d: an integer from [0,5] representing a direction in RULE_ORDER
    :return: the index in RULE_ORDER of the negated vector RULE_ORDER[d]
    """
    if d % 2 == 0:
        return (d + 1) % 6
    else:
...
# kinda wish these next two were in a different module
def get_orientation(face_idx: int,
                    ori_idx: int) -> np.ndarray:
    """
    :param face_idx: an index in RULE_ORDER
    :raise ValueError: if face_idx is not a valid face index (face_idx \u2208 [1,5])
    :param ori_idx: the orientation index
    """
    if face_idx >= len(RULE_ORDER) or face_idx < 0:
        raise ValueError(f"{face_idx} is not an index in RULE_ORDER")
...
def getRotationMap(axis: int, angle: int) -> dict[int, int]:
    """
    Converts an axis-angle rotation (as integer indeces) to a rotation map (direction index key-value pairs)

    :param axis: an integer from [0,5] representing a direction in RULE_ORDER
    :param angle: a rotation angle as an integer from [0,3] which is multiplied by \u03C0 / 2 to conver to a radian angle

    :return: a rotation map of length 6 where keys are nonrepeating integers [0,5] and values are nonrepeating integers [0,5]

    todo is this even right
    """
    return {
        d: get_orientation(axis, (d + angle) % 4)
        if d not in (axis, rdir(axis)) else d
        for d, _ in enumerate(RULE_ORDER)
    }
...
class PolycubesPatch(BasePatchType):
    ...
    def __init__(self,
                 ...
                 orientation: Optional[Union[int, np.ndarray]]=None,
                 stateVar: int = 0,
                 activationVar: int = 0):
    ...
    def rotate(self, rotation: np.ndarray) -> PolycubesPatch:
        ...
        p = PolycubesPatch(self.get_id(),
                           ...
                           np.matmul(rotation, self.alignDir()).round() if self.has_torsion() else None,
                           self.state_var(),
                           self.activation_var())
        p.check_valid()
        return p
    ...
    def check_valid(self):
        assert self.color()
        assert not self.has_torsion() or np.dot(self.direction(), self.alignDir()) == 0
    ...
    def remove_torsion(self):
        self._key_points = [self.direction()]


class PolycubeRuleCubeType(PatchyBaseParticleType):
    """
    A cube type in a polycube rule
    """
    def __init__(self,
                 ct_id: int,
    ...
    def to_string(self, force_static=False) -> str:
        ...
        # else:
            # lmao
        sz = "|".join([
            self.get_patch_by_diridx(i).to_string() if self.has_patch(i) and self.patch(i).color() else ""
...
class PolycubesRule(BaseParticleSet):
    ...
    def toJSON(self) -> list:
        return [{
            ...
            "patches": [{
                ...
                # insane syntax
                **({'alignDir': to_xyz(p.alignDir())} if p.has_torsion() else {}),
                "color": p.color(),
    ...
    def add_particle_patch(self, particle: Union[int], patch: PolycubesPatch):
        assert patch.color(), f"Patch color should be nonzero! Passed value: {patch.color()}"
    ...
    def to_SAT(self, filename: Union[Path, str]):
        ...
        with filename.open("w") as f:
            f.write(self.to_SAT_str())

    def to_SAT_str(self) -> str:
        return "\n".join(self.to_sat_list())

    def to_sat_list(self) -> list[str]:
        sat_vars = []
        colorcount = 0
        ...
        for i_particle, particle_type in enumerate(self.particles()):
            for patch in particle_type.patches():
                if patch.color() not in colormap:
                    ...
                    colorcount += 1
                sat_vars.append(f"C({i_particle},{patch.dirIdx()},{colormap[patch.color()]})")
        for color in self.color_set():
            if color > 0:
                assert -color in colormap
                sat_vars.append(f"B({colormap[color]},{colormap[-color]})")
        return sat_vars
    ...
    @functools.cache
    def to_canonical(self) -> PolycubesRule:
        """
        converts the rule to a standardized "canonoical" form by rotating cube types, changing cube type order,
        and color ordering
        This is useful for comparing rules that have the same pattern of interactions expressed in different color
        enumerations and cube type rotations
        function written in part by catGPT but I had to refise heavily
        """

        # Step 1: Count all color occurrences
        ...
        # Step 2: Build deterministic color map based on frequency then value
        sorted_colors = sorted(color_counts.items(), key=lambda x: (-x[1], x[0]))
        color_map = {}
        next_color = 1
        for original_color, _ in sorted_colors:
            if original_color > 0 and original_color not in color_map:
                color_map[original_color] = next_color
                color_map[-original_color] = -next_color
                next_color += 1

        # Step 3: Apply color remapping to a copy of the rule
        recolored = copy.deepcopy(self)
        for patch in recolored.patches():
            c = patch.color()
            if c != 0:
                patch.set_color(color_map[c])

        # Step 4: Canonicalize cube orientation by finding minimal string over all rotations
        cube_rotations = [np.round(r).astype(int) for r in R.create_group('O').as_matrix()]
        canonical_cubes = []
        for cube in recolored.particles():
            best_str = None
            best_cube = None
            for r in cube_rotations:
                cpy = copy.deepcopy(cube)
                cpy.rotate(r)
                s = cpy.to_string(force_static=False)
                if best_str is None or s < best_str:
                    best_str = s
                    best_cube = cpy
            canonical_cubes.append(best_cube)

        # Step 5: Sort canonical cubes by string representation
        canonical_cubes.sort(key=lambda c: c.to_string(force_static=False))

        # Step 6: Build the final canonical rule
        canonical_rule = PolycubesRule(nS=0)
        for cube in canonical_cubes:
            canonical_rule.add_particle(cube)
            for patch in cube.patches():
                patch.set_type_id(len(canonical_rule.patches()))
                canonical_rule.add_patch(patch)
