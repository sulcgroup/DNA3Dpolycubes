...
"""utility.py
================
General purpose helper functions used across the PyPatchy codebase.

The functions in this module are intentionally **stateless** and have **no
external side-effects**, which makes them easy to unit-test.  They range from
simple NumPy linear-algebra helpers (`dist`, `normalize`, `angle_between`) to
filesystem convenience wrappers (`get_local_dir`, `lsin`) and colour
utilities (`selectColor`).

Only the most commonly used public helpers are documented below – internal
implementation details may change at any time.

Public API
----------
* dist(a, b) – Euclidean distance between two vectors.
* normalize(v) – Return a unit-length copy of the vector `v`.
* angle_between(v1, v2) – Angle (radians) between two vectors.
* halfway_vector(a, b) – Unit vector halfway between `a` and `b`.
* random_unit_vector() – Pseudo-random 3-D unit vector.

(See inline docstrings for the remaining helpers.)
"""
...
if not (get_local_dir() / "settings.cfg").exists():
    raise ImportError(f"Missing settings.cfg file! expected to find file in {Path(get_local_dir() / 'settings.cfg')}")
...
def tlm_data_dir() -> Union[None, Path]:
    if "tlm_data_dir" in cfg["ANALYSIS"]:
        return Path(cfg["ANALYSIS"]["tlm_data_dir"])
    else:
        raise ValueError(
            "No data storage directory specifid in ~/.pypatchy/settings.cfg! Set with tlm_data_dir=path/to/my/directory")
...
def selectColor(number: int, saturation=50, value=65, fmt="hex") -> Union[str, np.ndarray]:
    # treat hue as an angle on a color wheel
    hue = number * 137.508  # use golden angle approximation
    hue = int(hue) % 360
    if fmt == "hsv":
        return f"hsv({hue},{saturation}%,{value}%)"
    else:
        r, g, b = hsv_to_rgb(hue / 360, saturation / 100, value / 100)
        if fmt == "rgb":
            return f"rgb({r},{g},{b})"
        elif fmt == "hex":
...
def rotAroundAxis(patchPos: np.ndarray, axis: np.ndarray, angle: float):
...
def rotation_from_to(v1: np.ndarray, v2: np.ndarray) -> R:
    """ Returns a scipy Rotation that rotates vector v1 to vector v2. """
    v1_u = normalize(v1)
    v2_u = normalize(v2)
    cross_prod = np.cross(v1_u, v2_u)
    dot_prod = np.dot(v1_u, v2_u)
    if np.isclose(dot_prod, 1.0):
        return R.from_quat([0, 0, 0, 1])  # No rotation needed
    elif np.isclose(dot_prod, -1.0):
        # 180 degree rotation around any axis perpendicular to v1
        perp_axis = np.array([1, 0, 0]) if not np.allclose(v1_u, [1, 0, 0]) else np.array([0, 1, 0])
        rotation_axis = normalize(np.cross(v1_u, perp_axis))
        return R.from_rotvec(np.pi * rotation_axis)
    else:
        angle = np.arccos(np.clip(dot_prod, -1.0, 1.0))
        rotation_axis = normalize(cross_prod)
        return R.from_rotvec(angle * rotation_axis)
