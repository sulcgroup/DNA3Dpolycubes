...
from ..patchy.base_param_val import ParameterValue
from ..patchy.ensemble_parameter import EnsembleParameter
...
class AnalysisPipelineStep(ABC):
    ...
    @abstractmethod
    def __call__(self, *args: Union[PipelineData, AnalysisPipelineStep]) -> PipelineData:
        """
        Executes this analysis step

        :return: data! (in a PipelineData wrapper object)
        """
        pass

    def get_cache_file_name(self) -> str:
        """
        :returns: the filename for where this step will cache data
        """
    ...
    def cache_data(self, data: PipelineData, file_path: Path):
        """
        Caches data to a file at file_path
        """
    ...
    def get_output_data_type(self) -> PipelineDataType:
        ...
        :returns: the data type produced by this pipeline, represented by an enum
        """
        pass
...
class AnalysisPipelineHead(AnalysisPipelineStep, ABC):
    ...
    def get_data_in_filenames(self) -> list[str]:
        """
        todo: try to cut this method out, it should be possible to find datafiles
        from args passed to the the __call__ method
        :return: a list of files containing the raw data that will be used by this step
        """
        pass
