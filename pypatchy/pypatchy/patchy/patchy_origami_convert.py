...
from ipy_oxdna.structure_editor.dna_structure import construct_strands, BASE_BASE, rc, POS_BASE, DNAStructureStrand, load_oxview, \
    load_dna_structure

import itertools

from ..origami_deabstractor import OrigamiDeabstractor
...
class PatchyOrigamiConverter(OrigamiDeabstractor):
    """
    ...
    def __init__(self,
                 ...
                 sticky_length: Union[None, int] = None,
                 padding: float = 1.0,
        ...
        # set parameters
        super().__init__(sticky_length, spacer_length=spacer_length, expected_num_edges=expected_num_edges)
    ...
    def get_dist_ratio(self) -> float:
        ...
        if self.dist_ratio is None:
            ...
            for p1, p2 in self.scene.iter_bound_particles():
    ...
    def assign_color_sequence(self, color: int, seq: str, update_rc=True):
        ...
        """
        if color not in self.scene.particle_types().interaction_matrix():
            print(f"Color {color} not in scene particle types interaction matrix. Ignoring...")
        else:
            ...
            if update_rc:
                interacting_colors = list(self.scene.particle_types().interaction_matrix().get_interacting_colors(color))
                assert len(interacting_colors) == 1
                # assert self.patchy_scene.particle_types().interaction_matrix()[(color, -color)], "Color is not set to interact with its negative pairing in the interaction matrix!!"
                self.color_sequences[-interacting_colors[0]] = rc(seq)
    ...
    def match_patches_to_strands(self,
        ...
        if self.scene.particle_types().has_udt_src():
            n_patches_udt = self.scene.particle_types().get_src().particle(p.type_id()).num_patches()
        else:
        ...
        if n_patches_udt < 3:
            ...
            for udt_id, strand_group_idx in relation.perm.items():
                ...
                patch_group = self.scene.particle_types().mdt_rep(udt_id)
    ...
    def patch_group_coords(self, p: PLPatchyParticle, dna: DNAParticle) -> tuple[dict[int, list[np.ndarray]],
        ...
        for patch in p.patches():
            ...
            if self.scene.particle_types().is_multidentate():
                patch_type_id = self.scene.particle_types().get_src_map().get_src_patch(patch).type_id()
            else:
    ...
    def bind_particles3p(self):
        ...
        for p1, p2 in self.scene.iter_bound_particles():
            ...
            assert p1_dna.linked_particle.get_uid() == p1.get_uid()
            assert p2_dna.linked_particle.get_uid() == p2.get_uid()
            found_bind_patch = False
            # iter pairs of patches that connect p1 to p2
            for patch1, patch2 in self.scene.iter_binding_patches(p1, p2):
                self.bind_patches_3p(p1_dna,
                                     patch1,
                                     p2_dna,
                                     patch2)
                found_bind_patch = True
            assert found_bind_patch, f"Particles {p1.get_uid()} and {p2.get_uid()} are identified as bound but have no binding patches!!"
    ...
    def bind_patches_3p(self,
        ...
        assert not self.is_track_conf_stickies() \
               or (particle1.linked_particle.get_uid(), patch1.type_id()) not in self.sticky_book,\
            f"already bound something to patch type {patch1.type_id()} on particle {particle1.linked_particle.get_uid()}"
        assert not self.is_track_conf_stickies() or\
               (particle2.linked_particle.get_uid(), patch2.type_id()) not in self.sticky_book,\
            f"already bound something to patch type {patch2.type_id()} on particle {particle2.linked_particle.get_uid()}"
    ...
    def convert(self, unbound_stickies: bool = True):
        ...
        """
        if self.is_track_conf_stickies() and len(self.sticky_book) > 0:
            print("WARNING: attempting call of convert() on a system where some sticky ends have already been assigned! "
                  "Program is probably about to crash.")
        assert len(self.scene.potentials) > 0, "You need to provide a patchy potential to use to identify patchy bonds"
        print("binding particles using 3p patches")
        self.bind_particles3p()
        assert self.expected_num_edges is None or self.expected_num_edges == -1 or self.bondcount == self.expected_num_edges, \
    ...
    # "convert" is actually a terrible name
    add_handle_linkings = convert
    ...
    def save_top_dat(self, write_top_path: Union[Path, str], write_conf_path: Union[Path, str]):
        ...
        if not write_top_path.is_absolute():
            if write_top_path.parts[0] == "~":
                write_top_path = write_top_path.expanduser()
            else:
        ...
        if not write_conf_path.is_absolute():
            if write_conf_path.parts[0] == "~":
                write_conf_path = write_conf_path.expanduser()
            else:
    ...
    def save_oxview(self,
                    write_oxview_path: Union[Path, str],
                    clusters=False,
                    color_by_type=False,
                    prettyprint_json=False,
                    seperate: bool = False):
        """
        ...
        Parameters:
            ...
            color_by_type whether to color the DNA particles according to patchy particle type (will be slower)
                    You can pass a custom dict where keys are particle type numbers and values are int rgb triplets
        """
        if isinstance(write_oxview_path, str):
            write_oxview_path = Path(write_oxview_path)
        if not write_oxview_path.is_absolute():
            if write_oxview_path.parts[0] == "~":
                write_oxview_path = write_oxview_path.expanduser()
            else:
        ...
        if not seperate:
            ...
            if color_by_type:
                for strand in oxview_json['systems'][0]['strands']:
                    for monomer in strand['monomers']:
                        ...
                        if isinstance(color_by_type, dict):
                            color = color_by_type[particle_type]
                        else:
            ...
            with write_oxview_path.open("w") as f:
                json.dump(oxview_json, f, indent=4 if not prettyprint_json else None)
            print(f"Wrote OxView file to {str(write_oxview_path)}")
        else:
            particles = self.get_particles()
            particle_posss = np.concatenate([p.poss() for p in particles], axis=0)
            poss_mins = particle_posss.min(axis=0)
            poss_maxs = particle_posss.max(axis=0)
            assert poss_mins.shape == (3,)
            for i, particle in enumerate(particles):
                particle.transform(tran=-poss_mins)
                particle.box = poss_maxs - poss_mins
                oxview_json = particle.get_oxview_json()
                if clusters or color_by_type:
                    for strand in oxview_json['systems'][0]['strands']:
                        for monomer in strand['monomers']:
                            particle_type = particle.linked_particle.type_id()
                            if isinstance(color_by_type, dict):
                            ...
                            # oxview file format requires we convert the hex code to base 10
                            monomer["color"] = int(color[1:], 16)
                if not write_oxview_path.exists():
                    write_oxview_path.mkdir()
                with (write_oxview_path / f"p{i}.oxview").open("w") as f:
                    json.dump(oxview_json, f,
                              indent=4 if not prettyprint_json else None)
    ...
    def iter_sticky_staples(self,
                            ...
                            incl_absent: bool = True) -> Generator[tuple[DNAParticle, int,
                                                                   ...
                                                                   None]:
        if pl_type is None:
            for p in self.scene.particle_types():
                if incl_absent or self.scene.particle_type_counts()[p.get_type()] > 0:
    ...
    def print_sticky_staples(self,
        ...
        for dna, strand_id, patch_id, strand in self.iter_sticky_staples(pl_type, incl_no_sticky, incl_absent):
            ...
            if patch_id is not None:
                print(f"Patch {patch_id} (Color {self.scene.particle_types().patch(patch_id).color()}) : " + sz)
            else:
                print(sz)
    ...
    def export_stickys_staples(self,
        ...
        for i, (dna, strand_id, patch_id, strand) in enumerate(self.iter_sticky_staples(incl_no_sticky=incl_no_sticky,
            ...
            seq_string_descriptor = f"{dna.linked_particle.name()}_Strand{strand_id}"

            if patch_id is not None:
                seq_string_descriptor += f"_Patch{patch_id}"
                if incl_original_patch_nums:
                    patch_src = self.scene.particle_types().get_src_map().get_src_patch(
                        self.scene.particle_types().patch(patch_id))
                    if is_polycube:
...
def show_scene_multigraph(scene: PLPSimulation):
    G = scene.compute_scene_multigraph(0)
    ...
    connectionstyle = [f"arc3,rad={r}" for r in itertools.accumulate([0.2] * max_edges)]
    ...
    labels = {
        ...
        tuple(edge): (f"{G.get_edge_data(*edge).get('e'):.2f}\n"
                      f"{'<->'.join(str(i) for i in list(zip(*G.get_edge_data(0,1,0).get('patches')))[1])}")
    ...
    plt.title("Patchy Conf Topology")
    plt.show()