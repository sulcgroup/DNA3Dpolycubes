...
class PLPatchyParticle(PatchyBaseParticleType, PatchyBaseParticle):
    ...
    cm_pos: np.ndarray
    _radius: float
    v: Union[None, np.ndarray]
    L: Union[None, np.ndarray]
    ...
    def __init__(self, patches: list[PLPatch] = [],
        ...
        self._radius = radius
        # if the particle is an instance, not a type
        # if self.get_uid():
        #     # ok, the "time to ruin my life about this" thing is that the "default" rotation is
        #     # [0  0  1]
        #     # [0 -1  0]
        #     # [1  0  0]
        #     # which, you will note, is not an identity matrix
    ...
    def has_patch_with_id(self, patch_id: int) -> bool:
        patches = [p for p in self.patches() if p.type_id() == patch_id]
    ...
    def normalize(self) -> PLPatchyParticle:
        """
        "normalize" a particle (type) by applying rotation to all patches and then setting rotation
        attributes to Null
        """
        cpy = copy.deepcopy(self)
        # if particle is already normalized, just return the copy
        if cpy.is_normd():
            return cpy
        ...
        assert np.allclose(cpy.rotmatrix(), np.identity(3)), "Failed to apply rotation matrix"
    ...
    def is_normd(self):
        # there should never be a situation where self.a1 is None but self.a3 isnt
        return self.a1 is None
    ...
    def rotation(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        assert self.a1 is not None, "Trying to get rotation matrix for a particle " \
                                    "which is implicitly not rotated (aka a particle type)"
    ...
    def set_rotation(self, param: np.ndarray):
        self.a1 = param[:, 0]
        self.a3 = param[:, 2]
        assert np.allclose(self.rotmatrix() @ self.rotmatrix().T, np.identity(3), atol=1e-5)
        assert abs(np.linalg.det(self.rotmatrix()) - 1) < 1e-4
