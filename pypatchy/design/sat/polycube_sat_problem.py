...
from .sat_problem import SATProblem, SATProblemPart, SATClause, exactly_one
from ...design.solve_utils import patchRotToVec, getFlatFaceRot, getIndexOf, patchVecToRot
...
class PolycubeSATProblem(SATProblem):
    ...
    def get_solution_var_names(self, solution: set[int]) -> list[str]:
        """
        :returns: a list of variable names that are true in the provided solution
        """
        return [vname for vname, vnum in self.variables.items() if vnum in solution]

    def check_problem_valid(self):
        """
        todo: implement
        """
        pass
    ...
    def add_constraints_all_patches(self):
        ...
        for c in range(self.nC):
            self.sat_problem_parts["^C"].add_clause([self.C(s, p, c)
                                                     for s in range(self.nS)
    ...
    def add_constraints_all_patches_except(self, forbidden: Iterable[int],
                                           nonRequired: Iterable[int] = [1]):
        """
        require that the solution use all colors except the ones in forbidden and nonRequired and
        specifically do not use colors in forbidden
        """
        self.add_problem_part(SATProblemPart("^C", ["s", "p", "c"]))
        ...
        for c in range(self.nC):
            if c not in forbidden and c not in nonRequired:
                self.sat_problem_parts["^C"].clauses.append(
                    [self.C(s, p, c) for s in range(self.nS) for p in range(self.nP)])
    ...
    def generate_BCO(self):
        ...
        if self.torsionalPatches:
            for s, p, o in itertools.product(range(self.nS), range(self.nP), range(self.nO)):
                self.O(s, p, o)
        # BCO also includes N, arrange the letters however you want
        if self.nNPT:
            if self.nNPT == 1:
                for s in range(self.nS):
                    self.N_single(s)
            else:
                for s, n in itertools.product(range(self.nS), range(self.nNPT)):
                    self.N_multi(s, n)
    ...
    def gen_legal_species_orientation(self):
        ...
        """
        self.add_problem_part(SATProblemPart("O", ["s", "p", "o{k}, o{l}"]))
        constraints = []
        if self.torsionalPatches:
            for s, p in itertools.product(range(self.nS), range(self.nP)):
    ...
    def gen_legal_patch_coloring(self):
        ...
        I think it may also be implicitly present in the
    ...
    def gen_forms_desired_structure(self, top: Iterable[tuple[int, int, int, int]]):
        """
        ...
        self.add_problem_part(SATProblemPart("BFi",
                                             ["l{i}",
                                              "p{i}",
                                              "c{i}",
                                              "l{j}",
        ...
        self.add_problem_part(SATProblemPart("DAi",
                                             ["l{i}",
                                              "p{i}",
                                              "o{i}",
                                              "l{j}",
        ...
        for l1, p1, l2, p2 in top:
            # color matches
            for c1, c2 in itertools.product(range(self.nC), range(self.nC)):
                self.sat_problem_parts["BFi"].clauses.append((-self.F(l1, p1, c1),
                                                              -self.F(l2, p2, c2),
                                                              self.B(c1, c2)))
            # species matches
            if self.torsionalPatches:
                for o1, o2 in itertools.product(range(self.nO), range(self.nO)):
                    self.sat_problem_parts["DAi"].clauses.append((-self.A(l1, p1, o1),
                                                                  -self.A(l2, p2, o2),
                                                                  self.D(p1, o1, p2, o2)))
    ...
    def gen_nanoparticle_singleparticle(self, np_locations: list[int]):
        ...
        """
        ppart_Z = SATProblemPart("Z",
                               ["l", "s", "r"])
        self.add_problem_part(ppart_Z)
        ppart_notN = SATProblemPart("~N",
                                    ["l, s, r"])
        self.add_problem_part(ppart_notN)
        # for each location
        for l in range(self.nL):
            ...
            for s in range(self.nS):
                if hasParticle:
                    ...
                    for r in range(self.nR):
                        ...
                        ppart_Z.clauses.append([self.N_single(s), -self.P(l, s, r)])
                else:
                    ...
                    for r in range(self.nR):
                        ...
                        ppart_notN.clauses.append([-self.N_single(s),
                                                -self.P(l, s, r)])
    ...
    def gen_req_mf_any_shared(self, top: Structure):
        problem_part = SATProblemPart("^M")

    def gen_req_mf_all_shared(self, top: Structure):
        """
        generates a problem part which will require that all cube types occur in both structures
        """
        problem_part = SATProblemPart("^M")
        # for each combination of species s and substructure
        for s, substructure in itertools.product(range(self.nS), top.iter_components()):
            # at least one position in the substructure must be occupied by the species, in any location
            problem_part.add_clause(list(itertools.chain.from_iterable([
                [
                    self.P(l, s, r) for r in range(self.nR)
                ] for l in substructure.vertices()
            ])))
        ...
        for i_particle, particle_type in enumerate(rule.particles()):
            for patch in particle_type.patches():
                if patch.color() not in colormap:
                    ...
                    colorcount += 1
                # isochoric heat capacity lol
                Cv = self.C(i_particle,
                            patch.dirIdx(),
                            colormap[patch.color()])
                assert Cv != len(self.variables)
                solution_vars.append(Cv)
                if "nO" in self.__dict__: # yikes
                    Ov = self.O(i_particle,
                                patch.dirIdx(),
                                patch.get_align_rot_num())
                    assert Ov != len(self.variables)
                    solution_vars.append(Ov)
    ...
    def problem_part_from_rule(self, rule: PolycubesRule, start_color_count=2) -> SATProblemPart:
        assert rule.num_particle_types() == self.nS, "Mismatch between rule and number of particle types!"
        assert rule.num_colors() + 2 == self.nC, "Mismatch between rule and number of species!"
        clause = SATProblemPart(str(rule))
        soln_vars = self.solution_from_rule(rule, start_color_count)
        for var in soln_vars:
            clause.clauses.append([var])
        return clause
    ...
    def save_problem_info(self, save_to: Path, bindings: Structure):
        """
        saves information about the SAT problem without saving all data
        """
        problem_info = {
            "nL": self.nL,
            "nC": self.nC,  # important to note: this is problem nC not solve spec nC
            "nS": self.nS,
            # skip nP, nO and nR
            "nNPT": self.nNPT,
            "torsion": self.torsionalPatches,
            "bindings": [list(binding) for binding in bindings]
        }
        with save_to.open("w") as f:
            json.dump(problem_info, f)

    # def visualize_solution(self, soln: PolycubeSATProblem):
    #     """
    #     show a solution as an image
    #     """
