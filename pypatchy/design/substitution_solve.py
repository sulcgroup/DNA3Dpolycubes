...
from .design_util import all_overlap_rotation, rotate_cube_type
from ..polycubeutil.polycube_util import get_fully_addressable_rule, coord_equal
...
def substitution_solve(s: FiniteLatticeStructure) -> PolycubesRule:
    ...
    new_rule_structure: PolycubeStructure = fully_addr_polycube
    # execute steps until we're no longer able to make an update
    while updated_rule_set:
        updated_rule_set = False
        new_rule_structure = try_simplify_rule(new_rule_structure)
        # if the new rule is shorter than the current rule, we've updated it so keep thiniing
        updated_rule_set = new_rule_structure.rule.num_particle_types() != fully_addr_polycube.rule.num_particle_types()

    return new_rule_structure.rule


def try_simplify_rule(structure: PolycubeStructure) -> PolycubeStructure:
        ...
        for ct2 in structure.rule.particles()[:ct1.type_id()]:
            ...
            if new_rule.num_particle_types() != structure.rule.num_particle_types():
                if libpolycubes.isBoundedAndDeterministic(str(new_rule), 100, 'stochastic'):
                    # this code will currently not work, fix it
