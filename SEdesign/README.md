...
# WARNINGS
`orthogonal_seq_screen_parallel.py` has a `--dG_file` flag that was made for one purpose, to allow you to change the number of output pairs and the number of optimization steps without rerunning the computation of the dG matrix. IF the dG file flat is not changed and ANY changes are made EXCEPT for changing number of output pairs or number of optimization steps, the script will FAIL.
