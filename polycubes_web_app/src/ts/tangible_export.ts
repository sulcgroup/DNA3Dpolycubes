// guide to matching up tabs
// match edges w/ same letter
...
import {addSpecies, FACE_ROT_OFFSETS, update} from "./edit";
import {cubeTypeFromJSON, CubeTypeJson} from "./rule";
...
type CutSettings = {
    // cube dimensions
    board_w_cm: number, // width of mat board in centimeters
    board_h_cm: number // width of mat board in centimeters
    board_thickness_cm: number // thickness of board in centimeters
    face_w_cm: number // width of cube face in centimeters
    patch_w_cm: number // width of interaction site (3d printed) in cm
    patch_h_cm: number // height of interaction site (3d printed) in cm

    // other cut setting
    n_tabs: number // number of tabs to cut into the side of each face
};
...
const CUT_DEFAULTS: CutSettings = {
...
function canvasDimensions() : [number, number] {
    return [settings.board_w_cm * PX_PER_CM, settings.board_h_cm * PX_PER_CM];
}

function faceCounterPerBoard() : [number, number] {
...
function cubeFaceInnerW() : number {
    /**
 ...
 */
export function drawCutterWindow(cubes: DefaultCubeInst[]) {
    ...
    let cubeColorCounts = new Map<number, number>();
    ...
    $("#lcut-upload-button").on('click', (e: JQuery.Event)=> {
    ...
    $("#lcut-download-button").on('click', (e: JQuery.Event)=>{
        // loop svg elements
    ...
    $("#lcut-close-button").on('click', (e: JQuery.Event)=>{
...
function* getCubesLCutSvg(cubes: DefaultCubeInst[]): Generator<SVG.Svg> {
    ...
    let cubes_queue = new Queue<DefaultCubeInst>(...cubes);
    ...
    while (cubes_queue.length > 0) {
        ...
        for (let patchIdx = 0; patchIdx < RULE_ORDER.length; patchIdx++) {
            ...
            face_group.translate(
                ...
                (patch_idx_x * (settings.face_w_cm - settings.board_thickness_cm)) * PX_PER_CM,
                (patch_idx_y * (settings.face_w_cm - settings.board_thickness_cm) + settings.board_thickness_cm) * PX_PER_CM);
 ...
 */
function drawCubeLCutFace(cube: PolycubeCubeInst<any>,
                             patch_idx: number,
                             canvas: SVG.Container) : SVG.Container {
    let pathData = "M 0,0 ";
    ...
    for (let i = 0; i < settings.n_tabs; i++) {
        ...
        face_group.text(tab_labels[2])
            .center(
                cubeFaceInnerW() - tabWidth * (i + 0.75),
                cubeFaceInnerW())
            .fill('none')
            .stroke(STROKE_DRAW)
            .font(TAB_LABEL_FONT)
    ...
    if (patch.color != 0) {
        ...
        const patch_angle: number = -signed_axis_angle(FACE_ROTATIONS[patch_idx], patch.alignDir, RULE_ORDER[patch_idx])
        const face_rot_idx: number = patch_angle * (2 / Math.PI);
        ...
        face_group.ellipse(settings.patch_w_cm * PX_PER_CM, settings.patch_h_cm * PX_PER_CM)
            .move(
                cubeFaceInnerW() / 2 - settings.patch_w_cm / 2 * PX_PER_CM,
                cubeFaceInnerW() / 2 - settings.patch_h_cm / 2 * PX_PER_CM)
            .rotate(rotation)
            .stroke(STROKE_CUT)
            .fill("none");
        ...
        face_group.text(`Color: ${patch.color}`)
            .center(
                settings.face_w_cm * PX_PER_CM / 2,
                settings.board_thickness_cm * PX_PER_CM + LABEL_TEXT_FONT.size * 1.25)
            .rotate(rotation + 180, // use overall face w here, not inner
                (settings.face_w_cm - settings.board_thickness_cm) * PX_PER_CM / 2,
                (settings.face_w_cm - settings.board_thickness_cm) * PX_PER_CM / 2)
            .stroke(STROKE_DRAW)
            .fill('none')
