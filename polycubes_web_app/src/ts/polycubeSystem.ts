...
import {DefaultCubeInst, PolycubeCubeInst} from "./cube_instance";
...
export class PolycubeSystem extends Polycube{
	...
	protected staging: Array<AssemblyComponentList<PolycubeCubeType>>;
    ...
    constructor(nMaxCubes=1000,
		...
		this.staging = [new AssemblyComponentList<PolycubeCubeType>(0)];
	 ...
	 * @param cubeType
	 */
	public addCubeType(cubeType: PolycubeCubeType, update_piecewise: boolean = true) {
	...
	allowedCubeTypes(stage: number | undefined= undefined): AssemblyComponentList<PolycubeCubeType> {
	...
	removeCubeType(ct: PolycubeCubeType | number) {
	...
	clone(){
		let copy = super.clone() as PolycubeSystem;
	 ...
	 */
	clear_staging() {
		this.staging = [new AssemblyComponentList<PolycubeCubeType>(0, [...this.cube_types])];
	}
    ...
    reset(reset_random=true) {
		...
		this.steps = 0;
		this.current_stage = 0;
		this.cubes = new Map<string, PolycubeCubeInst<any>>();
    ...
    set_rule(rules : PolycubeCubeType[] = [], reset_random=true) {
	...
	refresh_piecewise_staging() {
		...
		for (let iStage = 0; iStage < this.numStages(); iStage++){
			...
			pieces = this.iterPiecewiseAssembly(pieces,
				this.allowedCubeTypes(iStage).options(),
				this.maxPieceSize);
    ...
    protected addPiece(p: PolycubeSystem, stage=0){
		this.piecewise_staging[stage].add(p);
	}

	protected addPieces(stage: number = 0, ...pieces){
	 ...
	 * @param maxDepth
	 */
	private iterPiecewiseAssembly(pieces: Polycube[],
								   cubeOptions: PolycubeCubeType[],
								   maxDepth: number) : Polycube[] {
					...
					cubeColors.get(c).forEach(ct => {
						...
						ct.patches.forEach((patch, i) => {
							...
							if (this.compatibleColors(m.color, patch.color)) {
								...
								// the same direction:
								let cube = this.instantiateCube(ct, this.genCubeName(), m.pos).rotateInstanceFromTo(
									RULE_ORDER[patch.direction],
									m.direction);
								// get direction index
								let i = RULE_ORDER.indexOf(m.direction);
								// if torsion is off, match patch align directions
								if (this.torsion) {
									let theta = getSignedAngle(
										m.alignDir,
										cube.patch_by_local(i).alignDir,
										RULE_ORDER[i]
									);
									// apply rotatiion
									...
									console.assert(!this.torsion || m.alignDir.distanceTo(cube.patch_by_local(i).alignDir) < 1e-5,
										`Angle mismatch between ${vecToStr(m.alignDir)} and ${vecToStr(cube.patch_by_local(i).alignDir)}`);
									...
									[...Array(4).keys()].forEach(rotIdx => {
										...
										cube.rotateInstanceAroundAxis(m.direction, theta);
										console.assert(!this.torsion || m.alignDir.distanceTo(cube.patch_by_local(i).alignDir) < 1e-5,
											`Angle mismatch between ${m.alignDir} and ${cube.patch_by_local(i).alignDir}`);
	 ...
	 * @param c
	 */
	matchingColors(c: number) : number[] {
		return [-c];
	}
	 ...
	 */
    public fitCube(move: Move, cube: PolycubeCubeInst<any>) : PolycubeCubeInst<any> | false {
	...
	public fitCubeFace(move: Move, cube: PolycubeCubeInst<any>, face: number) : PolycubeCubeInst<any> | false {
		// Rotate cube so that the matching face has
		// the same direction:
		cube = cube.clone();
		console.assert(this.compatibleColors(move.color, cube.patch(face).color),
			`Move color ${move.color} does not match patch color ${cube.patch(move.direction).color}`)
		cube.rotateInstanceFromTo(
			RULE_ORDER[face],
			move.direction);
		...
		console.assert(!this.torsion || move.alignDir.distanceTo(patch_new_align) < 1e-5,
			`Angle mismatch between ${vecToStr(move.alignDir)} and ${vecToStr(patch_new_align)}`);
	 ...
	 */
	public canPlaceCube(c: PolycubeCubeInst<any>) : boolean {
	 ...
	 */
    pieceFits(move: Move, piece: PolycubeSystem) : false | PolycubeSystem  {
        // Traverse move faces and piece faces in random order
		let piece_moves: Move[] = piece.allMoves();
        let rc: number[] = randOrdering(piece.moves.size, this.rng);
     ...
     */
    testPieceMove(move: Move,
				  piece: PolycubeSystem,
				  piece_move: Move) : false | PolycubeSystem {
        // If we find an equal color
...
//            let piece_translated = piece.translate(t_initial);
		...
		q.setFromUnitVectors(piece_move.direction,
							 move.direction.clone().negate());
...
//            console.assert(rotated_move_key.clone().applyMatrix4(t).equals(move.pos));
		...
		console.assert(this.compatibleColors(move.color,
...
//            console.assert(this.compatibleColors(move[i].color, rule_rotated.colors[i]));
	 ...
	 */
    tryProcessSingleCubeMove(move: Move, cubeType: PolycubeCubeType): false | DefaultCubeInst {
		let cube: PolycubeCubeInst<any> | false = this.instantiateCube(cubeType, this.genCubeName(), move.pos.clone());
    ...
    tryProcessPieceMove(move: Move, piece: PolycubeSystem) : false | PolycubeSystem {
        ...
        if(fitted_piece) { // if a rule is identified that fits the move
			[...fitted_piece.cubes.values()].forEach((c) => {
				let cube: DefaultCubeInst = this.instantiateCube(c.getType(), this.genCubeName(), c.position);
	...
	getValidPieces(move: Move, stage?) : AssemblyComponentList<PolycubeSystem>{
		if (stage == void 0){
			stage = this.current_stage;
		}
    ...
    public step (){
		...
		let move: Move = this.moveFromIdx(Math.floor(this.rng() * n_moves));
	 ...
	 */
	getAllPossibleMoveOrders(stage?: number) : AssemblyComponentList<PolycubeCubeType>[][]{
		if (stage == void 0){
			stage = 0;
		}
		...
		let assembly_order : AssemblyComponentList<PolycubeCubeType>[][] = [...range(num_assembly_orders)].map(
			() => [...range(this.maxPieceSize)].map(
				() => new AssemblyComponentList<PolycubeCubeType>(stage)
			)
		);
	...
	transform(t: THREE.Matrix4) : PolycubeSystem{
		let copy = this.clone() as PolycubeSystem;
	...
	updateMove(cube: PolycubeCubeInst<any>, face: PolycubePatch) {
		let movePos: Vector3 = cube.position.clone().add(RULE_ORDER[face.direction])
