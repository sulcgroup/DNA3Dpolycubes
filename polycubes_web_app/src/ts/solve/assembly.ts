...
import {RULE_ORDER, shuffleArray, strToVec} from "../utils";
import {PolycubeCubeType} from "../rule";
...
export class AssemblyComponentList<T extends (PolycubeCubeType | Polycube)> implements Iterable<T> {
    [Symbol.iterator]() {
        let counter = 0;
        return {
            next: () => {
...
export class PiecewiseStage extends AssemblyComponentList<Polycube>{
    colormap: Map<number, AssemblyComponentList<Polycube>>;
    all_pieces: AssemblyComponentList<Polycube>;

    constructor(idx: number) {
        super(idx);
        this.colormap = new Map();
        this.all_pieces = new AssemblyComponentList<Polycube>(idx);
    }
            ...
            move.forEachFace(f => {
                ...
                if (f) {
                    ...
                    if (!(-f.color in this.colormap)) {
                        ...
                        this.colormap[-f.color] = new AssemblyComponentList<Polycube>(this.index);
                    }
 ...
 * @param cube_types
 */
export function cubeSetColorMap(cube_types: PolycubeCubeType[]) : Map<number, PolycubeCubeType[]>{
    let cube_map = new Map<number, PolycubeCubeType[]>();
