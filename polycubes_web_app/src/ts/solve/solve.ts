...
let rollOverMaterial: MeshLambertMaterial = new THREE.MeshLambertMaterial({
...
const sphereGeo: SphereGeometry = new THREE.SphereGeometry(.35, 32,16);
...
let voxels: Mesh[] = [];
let voxel_map: Map<string, Mesh> = new Map();
let connectors: Map<string, any> = new Map();
...
let extraConnMap: Map<[string, number], ExtraConnection> = new Map(); // maps position strings to ExtraConnection objects
 ...
 * @param position
 */
function getVoxel(position: Vector3): Mesh | null {
...
function addVoxel(position: Vector3) {
...
function saveSolveSpec() {
    ...
    function jsonExportReplace(key: string, val: any){
        if (key == "bindings"){
    ...
    saveString(
        JSON.stringify(out, jsonExportReplace, 2),
        "polysat.json"
    );
}
...
function loadSolveSpec(solveSpec: SolveSpec) {
    clearTopology();
...
function handleFile(file = (document.getElementById('load') as HTMLInputElement).files[0]) {
    new Response(file).json().then(json => {
        ...
        if ("nanoparticles" in json && json["nanoparticles"].length > 0){
            [...json["nanoparticles"].entries()].forEach(([key, val]: [string, number]) => {
...
function getFullyAdressableRule() : PolycubeCubeType[]{
 ...
 */
function addInternalConnection(c1: Vector3, c2: Vector3) {
...
function onDocumentMouseDown(event: MouseEvent) {
    if (event.button == 0) {
        event.preventDefault();
        window.mouse.set((event.clientX / window.innerWidth) * 2 - 1,
        ...
        if (intersects.length > 0) {
			...
			// add nanoparticle
                ...
                if (activeNPType > -1){
					...
					// if no nanoparticle is already at this position
						...
						addNanoparticle(i.object as Mesh, activeNPType);
					}
				}
	        ...
	        // create add external connection
				...
				addCrystalConnection(i.object as Mesh, i.face.normal);
			// create cube
			} else {
...
function addNanoparticle(voxel: Mesh, iType: number){
    ...
    (voxel.material as Material).transparent = true;
    (voxel.material as Material).opacity = 0.1;
	window.scene.add(np);
	npTypeCounts[`${iType}`]++;
...
function addCrystalConnection(voxel: THREE.Mesh, dir: Vector3){
...
*/
function smartEnumerate(xMax: number, yMax: number, xMin=1, yMin=1) : [number, number][] {
    let l = []
...
probably?
*/
function countParticlesAndBindings(topology: [number,number,number,number][]) {
...
$("#generate-crystal").on("click", function (){
    generateCrystalLattice(
        Number($("#crystalX").val()), // joke of a language
        Number($("#crystalY").val()),
        Number($("#crystalZ").val())
    );
});
 ...
 * @param height
 */
function generateCrystalLattice(width: number, height: number, depth: number){
                ...
                for (let direction of RULE_ORDER){
                    if (getVoxel(new Vector3(x,y,z).add(direction))) {
                        addInternalConnection(
                            new Vector3(x, y, z),
                            new Vector3(x, y, z).add(direction)
                        );
                    }
                }
            }
    ...
    for (let x of range(width)){
        for (let y of range(height)){
            for (let [z, dir] of [
                ...
                addCrystalConnection(getVoxel(new Vector3(x, y, z as number)), dir as Vector3);
            }
        }
    }
    ...
    for (let x of range(width)){
        for (let z of range(depth)){
            for (let [y, dir] of [
                ...
                addCrystalConnection(getVoxel(new Vector3(x, y as number, z)), dir as Vector3);
            }
        }
    }
    ...
    for (let y of range(height)){
        for (let z of range(depth)){
            for (let [x, dir] of [
                ...
                addCrystalConnection(getVoxel(new Vector3(x as number, y, z)), dir as Vector3);
            }
        }
    }
    render();
