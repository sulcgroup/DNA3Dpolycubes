...
import { cubeTypeColor, patchColor } from "./rule";
import { MeshLambertMaterial, Quaternion, Vector3 } from "three";
...
export class VisualPolycubeSystem extends PolycubeSystem {
    ...
    gridwidth;
    gridGroup;
    constructor(scene, nMaxCubes = 1000, maxCoord = 100, buildConfmap = true, torsion = true, envbrowser = $("<div>"), maxPieceSize = 1, rand_seed = "abcd") {
        ...
        this.connectorCubeGeo = new THREE.BoxGeometry(connectorCubeSize, connectorCubeSize, connectorCubeSize);
        this.connectorPointerGeo = new THREE.BoxGeometry(connectorCubeSize / 2, connectorCubeSize / 2, connectorCubeSize / 2);
        this.centerCubeGeo = new THREE.BoxGeometry(this.cubeSize, this.cubeSize, this.cubeSize);
    ...
    clone() {
        let copy = new VisualPolycubeSystem(false, this.nMaxCubes, this.maxCoord, this.confMap === undefined, this.torsion, this.envbrowser, this.maxPieceSize, this.rng_seed);
    ...
    addParticle(cube_instance) {
        ...
        // draw cube visual
        this.objGroup.add(cube_instance.draw(this.connectorCubeGeo, this.particleMaterials.get(cube_instance.type_id()).clone(), this.centerCubeGeo));
        ...
        cube_instance.envvis.mouseenter(function (e) {
            ...
            cube_instance.cube_vis.material.color = c.offsetHSL(0, 25, 0);
            render();
        });
        cube_instance.envvis.mouseleave(function (e) {
            cube_instance.cube_vis.material.color = new THREE.Color($(e.currentTarget).css('border-color'));
            render();
        });
    ...
    makeLine(a, b, color) {
        ...
        let mat_color = patchColor(color);
        material.color.set(mat_color);
        return new THREE.Line(geometry, material);
    }
    instantiateCube(cube_type, name, position = new Vector3(0, 0, 0)) {
        return new VisualPolycubeCubeInst(cube_type, position, Quaternion.prototype.identity(), name);
    }
    updateMoves(cube) {
...
export function initScene() {
    ...
    window.canvas = document.getElementById("threeCanvas");
