...
import { cubeTypeColor, cubeTypeFromJSON, DynamicPolycubeCubeType, FACE_NAMES, parseDecRule, patchColor, ruleToDec, } from "./rule";
...
export function addSpecies(new_species) {
    if (new_species == undefined) {
        // new_species = [];
        let ctName = window.system.nameNewCubeType();
        new_species = new DynamicPolycubeCubeType(window.system.numCubeTypes(), FACE_NAMES.map((x, i) => new PolycubePatch(0, i, FACE_ROTATIONS[i])), ctName);
    }
    ...
    let e_autoupdate = document.getElementById("autoUpdate");
    ...
    for (let i = 0; i < RULE_ORDER.length; i++) {
        ...
        color_field.addEventListener("change", updateCubeColor.bind(event, color_field, new_species, i));
        ...
        if (window.system.isPolycubeSystem()) { //under most operational circumstances (as of this writing)
            ...
            rotation.addEventListener("click", updatePatchRot.bind(event, rotation, new_species, i));
    ...
    remove.alt = "Remove";
    remove.addEventListener("click", removeCubeType.bind(event, new_species, ruleField));
    ...
    duplicate.addEventListener("click", duplicateCubeType.bind(event, new_species, ruleField));
    ...
    edit.addEventListener("click", editCubeType.bind(event, new_species, ruleField));
...
function updateCubeColor(e, new_species, faceIdx) {
...
function updatePatchRot(e, new_species, faceIdx) {
...
function removeCubeType(rule, ruleField) {
    let ncts = window.system.numCubeTypes();
    ruleField.parentNode.removeChild(ruleField);
    window.system.removeCubeType(rule);
    console.assert(ncts - 1 == window.system.numCubeTypes(), "Appear to have removed more than one cube types?");
...
function duplicateCubeType(rule) {
    // let ruleIdx = window.system.cube_types.indexOf(rule);
    let copy = rule.clone(window.system.numCubeTypes());
    window.system.addCubeType(copy);
    addSpecies(copy);
    let cubeMaterial = new THREE.MeshLambertMaterial({
        color: cubeTypeColor(window.system.numCubeTypes() - 1)
    });
    window.system.particleMaterials.set(copy.type_id, cubeMaterial);
    if (document.getElementById("autoUpdate").checked) {
        update();
    }
}
...
function editCubeType(cube_type, ruleField) {
    ...
    for (let i = 0; i < RULE_ORDER.length; i++) {
        ...
        if (window.system.isPolycubeSystem()) {
            ...
            rotation.addEventListener("click", e => {
                let rot = (parseInt(e.target.value) + 1) % 4;
        ...
        color_input.addEventListener("change", e => {
            ...
            e.target.parentElement.style.backgroundColor = c;
        ...
        if ("conditionals" in cube_type) {
            ...
            logic.addEventListener("change", e => {
                staged_allostery_alterations[i] = e.target.value;
            });
        ...
        // dynamic formualtion
            ...
            let state_var = $("<td>").append($("<input>").val(cube_type.patches[i].state_var).attr({
                "type": "number",
                "step": "1"
            }).on("input", function () {
                ...
                staged_allostery_alterations.patches[i].state_var = parseInt(this.value);
            }));
            let activation_var = $("<td>").append($("<input>").val(cube_type.patches[i].activation_var).attr({
                "type": "number",
                "step": "1"
            }).on("input", function () {
                if (!(i in staged_allostery_alterations.patches)) {
                ...
                staged_allostery_alterations.patches[i].activation_var = parseInt(this.value);
            }));
    ...
    function add_effect_ui(e, i) {
        return $("<tr>").append($("<td>").append($("<input>")
            ...
            if (pattern.test(this.value)) {
                ...
                let sources = this.value.split(",").map(x => parseInt(x));
            ...
            staged_allostery_alterations.effects[i].target = this.value;
        })));
    }
    ...
    if ("effects" in cube_type) {
        ...
        $(editbox).append($("<div>")
            ...
            .append($("<table id='ct-fx-list'>").append($("<tr><td>Source</td><td>Target</td></tr>"), ...cube_type.effects.map(add_effect_ui)), $("<button>+</button>").on("click", function () {
    ...
    apply_button.addEventListener("click", e => {
        ...
        if (flag_made_change) {
            window.system.refresh_piecewise_staging();
            update();
        }
...
// map of 2d grid coordinates for faces in the cutout
// index is face enumeration, numbers are x and y translation of face
export const FACE_SVG_COORDS = [
    [2, 2], //left
    [0, 2], //right
    [1, 0], //bottom
    [1, 2], //top
...
export function updateRuleVis() {
    ...
    rule_display.empty();
    var svg = SVG.SVG("#rule-display-canvas");
    big_cum_x = 0;
    big_cum_y = 0;
    let w_max = W_LIMIT;
...
function drawCubeType(svg, ct_idx) {
    let cubeType = window.system.cube_types[ct_idx];
    let f;
    let x = big_cum_x;
    let y = big_cum_y;
    let group = svg.group();
    ...
    // cumulative x coordinate
    let x_cum = FACE_SQUARE_W * 3 + 2 * FACE_FIG_PAD;
    function activates(x1, y1, x2, y2, deactivates = false, arc_r = 1.25) {
    ...
    function sets_state(x1, y1, state_idx, x2 = undefined) {
        let x3, y3;
...
export function import_system(jssys) {
    ...
    let rule = Object.entries(jssys.cube_types).map(([n, r], i) => {
        if (!("type_id" in r)) {
            r.type_id = i;
        }
        return cubeTypeFromJSON(r, r["typeName"]);
    });
    window.system = new VisualPolycubeSystem(window.scene, jssys.cubes.length, 100, //whatever
    false, //
    true, window.system.envbrowser); //new window.system
    // add all species
    ...
    jssys.cubes.forEach(c => {
        ...
        let cinst = window.system.instantiateCube(rule[ctidx], c["name"], cube_position);
        cinst.rotate(cube_rotation);
        cinst.set_state(c.state);
...
function download_topology() {
    ...
    saveString(JSON.stringify(out), 'topology.json');
}
...
export function resume(step_length = 0, src = undefined) {
    $(".stepctrlbtn").css("background-color", UNSELECTED_BTN_BG);
    ...
    function scheduleNextStep() {
        ...
        if (!window.system.isProcessingComplete()) {
...
function doStep() {
    ...
    let step_to = parseInt($("#stepto").val());
