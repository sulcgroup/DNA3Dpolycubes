...
$("#stepto").on("change", function () {
    window.system.setMaxCubes($("#stepto").val());
});
...
$("#pieceSize").on("change", function () {
    window.system.setMaxPieceSize($("#pieceSize").val());
    update();
});
    ...
    reader.onload = function (e) {
        if (typeof this.result === "string") {
            ...
            if (Array.isArray(rule_data)) {
                rule_data = rule_data.map((r, i) => {
                    if (!("type_id" in r)) {
                        r.type_id = i + window.system.cube_types.length;
                    }
                    return cubeTypeFromJSON(r, r["typeName"]);
                });
            }
            else {
                rule_data = Object.entries(rule_data).map(([n, r], i) => {
                    if (!("type_id" in r)) {
                        r.type_id = i + window.system.cube_types.length;
                    }
                    return cubeTypeFromJSON(r, r["typeName"]);
            ...
            let rule = Object.entries(rule_data).map(([n, r]) => cubeTypeFromJSON(r, r["typeName"]));
            // add all species
