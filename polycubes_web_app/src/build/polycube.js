...
import { PolycubeCubeInst, PolycubeSystemConnection } from "./cube_instance";
...
export class Polycube {
    ...
    // cubes
    cubes;
    cube_name_map; // maps cube personal names to cube instances
    currMinCoord;
    currMaxCoord;
    cube_types;
    cubeTypeCount;
     ...
     */
    listCubeTypes() {
        return this.cube_types.map((ct) => ct.clone(ct.type_id));
    }
    /**
     *
     */
    getCubeCoords() {
        return [...this.cubes.keys()].map((s) => strToVec(s));
    }
    /**
     *
     */
    ...
    allMoves() {
        ...
        const arrayOfMoveArrays = [...this.moves.values()].map(m => m.getMoves());
    ...
    hasMove(position) {
        if (position instanceof Vector3 && position.isVector3) {
            ...
            return this.moves.has(position);
        }
    }
    /**
     *
    ...
    hasCube(position) {
        if (position instanceof Object && position.isVector3) {
            ...
            return this.cubes.has(position);
        }
    }
    /**
     *
    ...
    instantiateCube(cube_type, name, position = new Vector3(0, 0, 0)) {
        return new PolycubeCubeInst(cube_type, position, Quaternion.prototype.identity(), name);
    ...
    updateMove(cube, patch) {
        // check if patch has colorx
        if (!patch.has_color()) {
            return; // nothing to do here
        }
        // find move position
        let movePos = cube.position.clone().add(RULE_ORDER[patch.direction]);
        ...
        let move_direction = RULE_ORDER[patch.direction].clone().negate();
        ...
        if (move_cell.speciesFit[dirIdx] != null) {
            console.assert((move_cell.speciesFit[dirIdx].color == patch.color) &&
                move_cell.speciesFit[dirIdx].alignDir.distanceTo(patch.alignDir) < 1e-5, `There's somehow already a move in position ${vecToStr(movePos)} facing direction ${dirIdx}`);
        }
        this.getMoveCell(movePos).setFace(move_direction, patch.color, patch.alignDir);
    }
...
//# sourceMappingURL=polycube.js.map