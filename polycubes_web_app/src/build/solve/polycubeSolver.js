...
import { DynamicPolycubeCubeType, ruleToDec } from "../rule";
import * as Logic from "logic-solver";
import { SolveSpec } from "./solveSpec";
import { getSysBehavior } from "../analysis";
...
export class polysat extends SolveSpec {
    ...
    constructor(topology = [], empty = [], nCubeTypes = -1, nColors = -1, nDim = 3, tortionalPatches = true) {
        ...
        this.variables = new Map(); // map of variable names (string) to variable indexes (int)
    ...
    add_constraints_all_patches_except(forbidden) {
        for (const c of range(this.num_colors())) {
            ...
            for (const p of range(this.nP)) {
                for (const s of range(this.num_species())) {
                    this.add_clause([-this.C(s, p, forbidden)]);
                }
            }
        }
    }
    ...
    add_constraints_nanoparticle_singletype() {
        // for each location
        for (const l of range(this.nL)) {
    ...
    add_constraints_nanoparticle_multitype() {
    ...
    readSolution(sol) {
        ...
        let solution_true_vars = sol.getTrueVars();
        let ruleMap = new Map();
        ...
        return [...ruleMap.values()].map((ct, i) => {
            return new DynamicPolycubeCubeType(i, [...ct.values()].map(patch => {
...
function patchVecToRot(i, v) {
    ...
    */
    let angle = getSignedAngle(FACE_ROTATIONS[i], v, RULE_ORDER[i]);
...
export function make_solver(spec) {
...
let minSol;
//# sourceMappingURL=polycubeSolver.js.map