...
import DynamicPolycubeCube, { PolycubePatch, ruleToDec } from "./rule";
// import {isBoundedAndDeterministic} from "./libs";
// @ts-ignore
import { t as minisatModule } from "src/libs/minisat.js";
//Polycube SAT Solver
class polysat {
    ...
    add_constraints_all_patches_except(forbidden) {
        for (const c of range(this.nC)) {
            ...
            for (const p of range(this.nP)) {
                for (const s of range(this.nS)) {
                    this.basic_sat_clauses.push([-this.C(s, p, forbidden)]);
                }
            }
        }
    }
...
function patchVecToRot(i, v) {
    ...
    */
    let angle = getSignedAngle(getFACE_ROTATIONS()[i], v, getRuleOrder()[i]);
...
' */
export function find_solution(topology, empty, nCubeTypes, nColors, nDim = 3, tortionalPatches = true) {
    // Initiate solver
