...
function simplify(rule) {
    ...
    let colorset = Array.from(new Set([].concat.apply([], rule.map(r => r.patches.map(f => {
...
export function getCoords(rules, nMaxCubes = 100, torsion = true, piece_size = 1 // pls do not
) {
    let sys = new PolycubeSystem(nMaxCubes, undefined, true, torsion, piece_size, randstr(12));
    sys.set_rule(rules);
    sys.seed();
    // run system
...
export function simplify2(rule, onUpdate) {
    ...
    const rotations = allRotations().map(r => [r,
                        ...
                        for (const [r, q] of rotations) {
                            if (compCols(coordsA, rotCoords(coordsB, r))) {
                                ...
                                if (is_bounded_deterministic &&
                                    coordEqual(getCoords(simplifiedNewRule), correctCoords)) {
                                    updatedRuleset = true;
                                    rule = newRule;
...
export function getSysBehavior(rule, nTries = 15, nMaxCubes = 1000, torsion = true, piece_size = 1, // please don't
stopIfNondet = false) {
    let coordGroups = [];
    let count = nTries;
    while (count--) {
