...
import { FACE_ROTATIONS, getUrlParam, getUrlVars } from "./utils";
import { DynamicPolycubeCubeType, parseDecRule, parseHexRule } from "./rule";
import { VisualPolycubeSystem } from "./view";
import { getRandSeed } from "./libs";
import { PolycubePatch } from "./patch";
export function createPolycubeSystem() {
    let cube_types = {};
    // Parse rule
    let vars = getUrlVars();
    if ("rule" in vars) {
        ...
        cube_types = defaultRule.map((cube_type, i) => {
            return new DynamicPolycubeCubeType(i, cube_type.map((c, j) => {
    ...
    let system = new VisualPolycubeSystem(window.scene, nMaxCubes, maxCoord, true, true, $("#env-browser"), 3, getRandSeed());
