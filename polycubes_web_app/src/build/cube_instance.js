...
import { cubeTypeColor, DynamicPolycubeCubeType, FACE_NAMES, patchColor } from "./rule";
import { FACE_ROTATIONS, RULE_ORDER, selectColor, vecToStr } from "./utils";
import faceShape from "../ui/face.svg";
export class PolycubeCubeInst {
    cube_type;
    position;
    rotation;
    personalName;
    ...
    patch(direction) {
        if (direction instanceof Vector3) {
            let direction_on_type = direction.clone().applyQuaternion(this.rotation.clone().invert());
     ...
     */
    correct_patch(p) {
        return { patch: p.rotate(this.rotation), srcdiridx: p.direction };
    }
    iter_patches(f) {
        [...this.get_patch_map().values()].forEach(f);
    }
    clone() {
        return new PolycubeCubeInst(this.getType(), this.position.clone(), this.rotation.clone(), this.getPersonalName());
    }
    rotate(q) {
     ...
     */
    hasPatch(idx) {
        let direction_on_type = RULE_ORDER[idx].clone().applyQuaternion(this.rotation.clone().invert());
        direction_on_type.round();
...
export class VisualPolycubeCubeInst extends PolycubeCubeInst {
    envvis;
    cube_vis;
    ...
    draw(connector_geometry, cube_mat, cube_geo) {
        ...
        this.iter_patches((p) => {
            ...
            if (p.patch.has_color()) {
                ...
                p.vis.big.position.add(RULE_ORDER[p.patch.direction].clone().multiplyScalar(0.4));
                // dimension of patch
                ...
                p.vis.big.scale.copy(new THREE.Vector3(1, 1, 1).sub(dim.multiplyScalar(0.65)));
    ...
    clone() {
        ...
        return new VisualPolycubeCubeInst(this.getType(), this.position.clone(), this.rotation.clone(), this.getPersonalName());
    }
    set_state(state) {
        this.state = state;
    }
...
//# sourceMappingURL=cube_instance.js.map